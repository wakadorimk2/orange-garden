...
**種類**: review
**正本**: このファイル（`docs/skills/review-preflight.md`）
**Codex用アダプタ**: `.codex/skills/review-preflight/SKILL.md`
**Claude用アダプタ**: `.claude/skills/review-preflight/SKILL.md`
**Codex 実行詳細**: [`docs/CODEX_RUNBOOK.md`](../CODEX_RUNBOOK.md) Appendix B

---

## Mission

merge 前の安全確認と 4 観点の意味的整合チェックを固定した順序で実施し、結果を報告する。
...
以下の順序で実施する。各ステップの失敗は停止せず記録し、全ステップ完了後に一括報告する。
自動修正・再試行ループは行わない。
...
3. lint チェックを確認する
4. テスト結果を確認する
5. 4 観点チェックを実施する（下記 Review Axes を参照）
6. Output Format に沿った報告を行う

> Note: Codex はステップ 1〜4 のコマンドを実行する。Claude は提供された実行結果のみを参照し、コマンドを実行しない。
