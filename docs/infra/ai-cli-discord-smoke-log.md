...
`NOTIFY_CHANNEL=discord-test` with `DISCORD_TEST_WEBHOOK_URL` so they fail
...
```bash
NOTIFY_CHANNEL=discord-test DISCORD_TEST_WEBHOOK_URL='<configured>' \
python3 scripts/codex_notify.py \
...
- the adapter reads `DISCORD_TEST_WEBHOOK_URL` or `~/.config/secrets/discord_test_webhook.env`
- if the test webhook is missing, it exits with code `2`
- it does not fall back to `DISCORD_WEBHOOK_URL`
