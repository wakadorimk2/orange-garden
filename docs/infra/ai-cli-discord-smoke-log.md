...
`scripts/codex_notify.py --smoke-test` or `notify --kind smoke_test` with
`DISCORD_WEBHOOK_AI_STATUS_TEST` so they fail closed instead of reaching the
prod webhook.
...
```bash
DISCORD_WEBHOOK_AI_STATUS_TEST='<configured>' \
python3 scripts/codex_notify.py --smoke-test \
...
```bash
DISCORD_WEBHOOK_AI_STATUS_TEST='<configured>' \
scripts/notify --kind smoke_test --title "issue #336 smoke" --source codex-tui "done"
```
