...
## scripts/ai_usage_monitor.py — 実行方法と既知制約
...
```bash
python scripts/ai_usage_monitor.py
```

5 分ごとに端末をクリアして使用状況を更新表示する。Ctrl-C で終了。

### 既知制約

| 項目 | 内容 |
|------|------|
| Claude 表示内容 | 制限到達状態は取得不能。当日の入出力トークン合計を表示する |
| Codex 表示内容 | `~/.codex/history.jsonl` の当日セッション数のみ。制限到達状態は取得不能 |
| ccusage | 任意。未インストール時は `~/.claude/projects/` 直接読取に自動フォールバックする |
| タイムゾーン境界 | ローカルタイムゾーン基準。サービス側 TZ と異なる場合、日次集計境界がずれる可能性がある |
| 端末クリア | `os.system("clear")` 利用。macOS / Linux で動作確認済み（Windows 未確認） |
| subagents ログ | `~/.claude/projects/<session>/subagents/*.jsonl` も集計対象だが、存在しないプロジェクトでは glob が空になる（正常動作） |
